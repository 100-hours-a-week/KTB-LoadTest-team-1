...
## 후속 조치 — 100 → 200 (Spring Boot 기본값 복원) ✅

200 VU 규모 부하테스트(`failedLoginScenario`가 모든 VU의 첫 스텝이라 회원가입/로그인에 요청이
몰림)에서 스레드 풀이 다시 병목이 되는지 실측했다. `auth-burst-thread-check.js`(동시 N개 VU가
각자 실패로그인→회원가입→로그인을 동시에 시작)로 `tomcat.threads.busy`를 폴링한 결과:
...
동시 VU 수랑 스레드 사용률이 거의 1:1로 비례하고, 100 근처부터 이미 포화 직전이었다. `100`은
Spring Boot/Tomcat 기본값(`200`)보다 낮게 잡혀 있던 값이라, 200으로 복원했다.

**재측정 결과** (동시 200 VU, 회원가입+로그인 burst):

| | max=100 | max=200 |
|---|---|---|
| 스레드풀 피크 | 100/100 (100%) | 193/200 (97%) |
| p99 지연시간 | 122ms | 100ms |
| 전체 소요 | 248ms | 183ms |

완전 포화는 피했지만 200 VU 규모에서도 97%로 여전히 여유가 크지 않다 — 인스턴스 하나의 스레드
풀을 더 올리는 것보다, t3.small(vCPU 2개) 기준 스레드를 과도하게 늘리면 메모리(스레드 스택)와
CPU(컨텍스트 스위칭)가 병목이 될 수 있어 **인스턴스를 여러 대로 늘리는 쪽이 다음 단계로 더
유효**할 것으로 판단했다([16번 항목](16-multi-instance-socketio.md)에서 멀티 인스턴스 정합성은
이미 처리함).
