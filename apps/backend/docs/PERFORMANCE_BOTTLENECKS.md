...
| [8](performance/08-redis-unused.md) | Redis가 인프라엔 있는데 코드에서 전혀 안 쓰임 | Medium (인프라) | ✅ 완료(옵트인) |
| [9](performance/09-ratelimit-transactional.md) | rate limit `@Transactional`이 사실상 무의미, 동시 요청 92% 유실 | Medium (인프라) | ✅ 완료(옵트인) |
| [10](performance/10-socketio-config.md) | Socket.IO 서버 설정(acceptBackLog/tcpNoDelay/단일노드) | Medium (인프라) | 🟡 부분 완료 |
| [11](performance/11-jvm-heap.md) | JVM 힙 기본값이 작다(`-Xmx1024m`) | Medium (인프라) | 미착수 |
| [12](performance/12-file-upload-io.md) | 파일 업로드가 동기 디스크 I/O | Medium | ✅ 완료 |
...
| [16](performance/16-multi-instance-socketio.md) | Socket.IO가 멀티 인스턴스에서 정합성이 깨지는 구조 | Critical(확장 시) | ✅ 완료(옵트인) |
...
| [9](performance/09-ratelimit-transactional.md) | 동시 요청 51건, rate limit 카운트 정확도(Mongo vs Redis 원자적 INCR) | Mongo 51건→**4건 집계(92% 유실)** vs Redis **51건→51건(0% 유실)** |
| [12](performance/12-file-upload-io.md) | 동시 20개 2MB 파일 업로드(기존 멀티파트 vs S3 presigned) | 서버 스레드 21→**3**, 처리 바이트 80MB→**20KB**, p99 3,980ms→**35ms** |
| [1번 후속](performance/01-tomcat-thread-pool.md) | 동시 200 VU 회원가입+로그인 burst, Tomcat 스레드풀(100 vs 200) | 피크 사용률 100%(포화) → **97%**, p99 122ms → **100ms** |
...
12. [12번] 파일 업로드 동기 디스크 I/O — **✅ 완료.** client→S3 presigned URL 방식으로 전환,
    e2e URL 호환을 위해 완료-확인 엔드포인트를 기존 URL에 `consumes`로 분기해 재사용.
13. [16번] Socket.IO 멀티인스턴스 정합성(`MemoryStoreFactory`/`LocalChatDataStore`가 JVM
    로컬이라 인스턴스 2대 이상에서 브로드캐스트/중복 로그인 감지가 깨지는 문제) — **✅ 완료.**
    원래 여기(감사 중 발견한 정합성 이슈)에 임시로 적어뒀던 항목을
    [16-multi-instance-socketio.md](performance/16-multi-instance-socketio.md)로 정식
    문서화하고, Redisson 기반 스토어(`socketio.store.type=redis`)로 해결.

### 3단계 — 인프라 레벨

14. [8][9번] 세션/rate limit을 Redis로 이전 — **✅ 완료(옵트인).** rate limit은 속도보다
    원자성이 핵심이었다(동시 요청 92% 유실 → 0% 유실, [9번](performance/09-ratelimit-transactional.md)).
    기본값은 여전히 mongo라 배포 시 프로퍼티를 켜야 적용된다.
15. [10번] Socket.IO `acceptBackLog`/`tcpNoDelay` 튜닝 — 연결 폭주 시나리오(`ramp-up-test.js`) 대비,
    아직 미착수(멀티인스턴스 스토어 부분은 [16번]으로 완료).
16. [11번] JVM 힙 크기 조정 — `apps/backend` 안에는 없음(`app-control.sh`가 systemd 서비스로
